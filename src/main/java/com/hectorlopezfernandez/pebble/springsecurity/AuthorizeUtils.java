...
final class AuthorizeUtils {
	...
	public static boolean authorizeUsingAccessExpression(String expression, ServletRequest request,
														 ServletResponse response) throws PebbleException {
		...
		FilterInvocation f = new FilterInvocation(request, response, (var request1, var response1) -> {
	...
	private static SecurityExpressionHandler<FilterInvocation> getExpressionHandler(ServletContext servletContext) {
		...
		throw new IllegalStateException("Configuration error. No visible WebSecurityExpressionHandler instance " +
				"could be found in the application context. There must be at least one in order to support " +
				"expressions in Pebble 'authorize' tags.");
	}
	...
	private static WebInvocationPrivilegeEvaluator getPrivilegeEvaluator(ServletRequest request) {
		...
		if (wipes.size() == 0) {
			throw new IllegalStateException("Configuration error. No visible WebInvocationPrivilegeEvaluator " +
				"instance could be found in the application context. There must be at least one in order to support " +
				"the use of URL access checks in Pebble 'authorizeUrl' tags.");
		}
